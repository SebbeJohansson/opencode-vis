/**
 * Shell scripts run through a one-shot PTY to fetch git snapshots (commit
 * contents, single-file before/after, whole worktree diff) as base64 blobs,
 * plus the parsers for their `##TAG` framed output.
 */
...
export function decodeCommitSnapshotBase64(value: string) {
  if (!value) return '';
